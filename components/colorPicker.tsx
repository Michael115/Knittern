...
export const ColorPicker: React.FC<Canvas> = ({width, height}) => {
    ...
    const draw = () => {
        if(canvasRef.current) {
            ...
            ctx.lineWidth = 1.5;
            ctx.arc(chosenColorPosition.current.x, chosenColorPosition.current.y, 10, 0, 2 * Math.PI);
            ctx.stroke();
        }
    }
    ...
    const mouseDown = (e: React.MouseEvent) => {
        ...
        var pt = adjustPointSpace(canvasRef.current, {x: e.clientX, y: e.clientY});
    ...
    const mouseDownColorBar = (e: React.MouseEvent) => {
        ...
        var pt = adjustPointSpace(colorBarCanvasRef.current, {x: e.clientX, y: e.clientY});     
